...
from schrodinger_engine.potentials.potentials_1D import Potential1D, TimeDependentPotential1D, Wall1D, HarmonicOscillator
...
class Simulation:
    ...
    def get_potential_energy_propagator(self, t):
        ...
        elif isinstance(self.potential, TimeDependentPotential1D):
    ...
    def solve(self) -> None:
        ...
        print(f"{solution[0, :].shape = }")
        # Processeurs d'évolution d'énergie cinétique et potentielle
        kinetic_propagator = self.get_kinetic_energy_propagator()
        potential_propagator = (self.get_potential_energy_propagator(t=0) if not isinstance(self.potential, TimeDependentPotential1D) 
                                else None)
        ...
        for i in tqdm(range(1, self.temporal_domain.N), desc="Simulation Progress", unit="time step"):
            ...
            if self.potential:
                if isinstance(self.potential, TimeDependentPotential1D):
    ...
    def display(self):
            ...
            if not self.potential is None:
                if not isinstance(self.potential, TimeDependentPotential1D):
                    # ax2 = ax.twinx()

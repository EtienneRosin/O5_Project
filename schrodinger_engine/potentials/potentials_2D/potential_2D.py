...
class Potential2D(ABC):
    """
    Base class for for 2D potentials. Must be subclassed to be usable.
    """
    @abstractmethod
    def __call__(self, x: float|np.ndarray, y: float|np.ndarray) -> float | np.ndarray:
        ...
        -------
        array-like
            V(x, y): potential at x.
        """
        pass
    
    def display(self, ax: plt.axes = None, x_range=(-15, 15), y_range=(-15, 15), num_points=2000):
        """
        ...
        lst_y = np.linspace(y_range[0], y_range[1], num_points)
        
        values = self.__call__(lst_x, lst_y)
...
class TimeDependentPotential2D(Potential2D):
    """
    Base class for time-dependent potentials.
    """
    @abstractmethod
    def __call__(self, x: float|np.ndarray, y: float|np.ndarray, t: float) -> float | np.ndarray :
        """
        ...
        -------
        array-like
            V(x, y, t): potential at (x,y) and time t.
        """
        pass
    
    def display(self, ax: plt.axes = None, x_range=(-15, 15), y_range=(-15, 15), num_points=200, T: float = 10) -> None:
        """
        ...
        lst_x = np.linspace(x_range[0], x_range[1], num_points)
        lst_y = np.linspace(y_range[0], y_range[1], num_points)
        values = self.__call__(lst_x, lst_y, lst_t)
